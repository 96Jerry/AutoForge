...
export class CronService {
  ...
  async updateGoogleCalendarWithWeeklyMeal() {
    // 식단표 크롤링
    const weeklyMealResult = await this.crawlService.getWeeklyMeal();

    // 식단표 분석
    const data = await this.googleService.analyze(weeklyMealResult.imagePath);
  ...
  /**
   * 매일 점심 12시 30분에 오늘의 메뉴를 Discord로 전송
   */
  @Cron('0 30 12 * * 1-5') // 매일 | 월~금 | 12시 30분 | 정각
  async sendTodayMenuToDiscord(): Promise<void> {
    try {
      ...
      // 식단표 분석
      const weeklyMenuData = await this.googleService.analyze(
        weeklyMealResult.imagePath,
      );
